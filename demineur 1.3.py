...
_ créer des variables générales regroupant les caractères utilisés, les tailles de grille, ...
_ renommer propremment les variables

¤ Démineur version 1.3 (Nettoyage des variables)
...
def generation_grille_vide () :
    ...
    """
    return [[None for j in range(nb_col)] for i in range(nb_lig)]
...
def emplacement_bombe (l = None, c = None) :
    ...
    if type(l) == int and type(c) == int and l >= 0 and l < nb_lig and c >= 0 and c < nb_col :
        indices.remove((l, c))
        emplacement_bombe_ameliore(l, c, indices)
...
def nb_bombes_adj (grille, l, c) :
    ...
    cpt = 0
    if l+1 < nb_lig :
        cpt += test_bomb(grille, l+1, c)
    if l-1 >= 0 :
        cpt += test_bomb(grille, l-1, c)
    if c+1 < nb_col :
        cpt += test_bomb(grille, l, c+1)
    if c-1 >= 0 :
        cpt += test_bomb(grille, l, c-1)
    if l+1 < nb_lig and c+1 < nb_col :
        cpt += test_bomb(grille, l+1, c+1)
    if l+1 < nb_lig and c-1 >= 0 :
        cpt += test_bomb(grille, l+1, c-1)
    if l-1 >= 0 and c+1 < nb_col :
        cpt += test_bomb(grille, l-1, c+1)
    if l-1 >= 0 and c-1 >= 0 :
        cpt += test_bomb(grille, l-1, c-1)
    return cpt
...
def generation_grille_jeu () :
    ...
    """
    return [[symb_cache for j in range(nb_col)] for i in range(nb_lig)]
...
def jeu () :
    ...
    continuer = True
    while continuer and not test_fin_ameliore(grille, solution):
        continuer = tour(grille, solution)
    affichage(grille)
    if not continuer :
        defaite_amelioree(grille, solution)
    else :
        victoire()
...
def test_fin (grille, solution) :
    ...
    Arguments : grille : une grille de jeu / solution : sa grille solution associée
    Valeur de retour : True si la grille est complète, False sinon.
    """
    for i in range(nb_lig) :
        for j in range(nb_col) :
            if not(grille[i][j].isdigit()) and (grille[i][j] != symb_detect or solution[i][j] != symb_bmb) :
                return False
    return True
...
def emplacement_bombe_ameliore (li, col, liste_cases) :
    # bug au test (qui devrait disparaître lors de l'implémentation définitive) - à bidouiller pour le test si possible
    """
    Retire d'une liste de coordonnées de cases les coordonnées des cases adjacentes à la case fournie en argument.
    
    Arguments : li : numéro de ligne / col : numéro de colonne / liste_cases : liste de coordonnées des cases de la grille.
    Valeur de retour : aucune.
    """
    cases = cases_adj(li, col)
    for c in cases :
        liste_cases.remove(c)
...
def affichage_nb_bombe (grille) :
    ...
    Valeur de retour : aucune.
    """
    print("Nombre de bombes détectées : ", calcul_nb_bombe(grille), "/", nb_bmb, ".")



def test_fin_ameliore (grille, solution) :
    """
...
def maj_bombes (grille, solution) :
    ...
    for i in range(nb_lig) :
        for j in range(nb_col) :
            if solution[i][j] == symb_bmb :
                grille[i][j] = solution[i][j]



def defaite_amelioree (grille, solution) :
    """
